...
VkPipeline CreateGraphicsPipeline(VkPipelineLayout pipelineLayout,
  ...
  VkShaderModule vertShaderModule = createShaderModule(
      "4_helloCompute/shaders/shader.vert.spv", device->GetVulkanDevice());
  VkShaderModule fragShaderModule = createShaderModule(
      "4_helloCompute/shaders/shader.frag.spv", device->GetVulkanDevice());
...
VkPipeline CreateComputePipeline(VkPipelineLayout pipelineLayout) {
  VkShaderModule compShaderModule = createShaderModule(
      "4_helloCompute/shaders/shader.comp.spv", device->GetVulkanDevice());
...
int main(int argc, char** argv) {
  ...
  modelTransforms.modelMatrix =
      glm::rotate(glm::mat4(1.f), static_cast<float>(15 * M_PI / 180),
  ...
  VkBuffer indexBuffer =
      CreateBuffer(device, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, indexBufferSize);
  unsigned int vertexBufferOffsets[2];
  VkDeviceMemory vertexBufferMemory =
      AllocateMemoryForBuffers(device, {vertexBuffer, indexBuffer},
  ...
  // Copy data to buffer memory
  {
    ...
    vkMapMemory(device->GetVulkanDevice(), vertexBufferMemory, 0,
                vertexBufferOffsets[1] + indexBufferSize, 0,
  ...
  BindMemoryForBuffers(device, vertexBufferMemory, {vertexBuffer, indexBuffer},
                       vertexBufferOffsets);

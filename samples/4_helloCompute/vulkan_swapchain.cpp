...
VulkanSwapChain::VulkanSwapChain(VulkanDevice* device, VkSurfaceKHR vkSurface)
  ...
  uint32_t imageCount =
      surfaceCapabilities.minImageCount + 1;  // 2 + 1 = triple buffering
  ...
  if (queueFamilyIndices[QueueFlags::Graphics] !=
    ...
    unsigned int indices[]         = {queueFamilyIndices[QueueFlags::Graphics],
                              queueFamilyIndices[QueueFlags::Present]};
    createInfo.pQueueFamilyIndices = indices;
  } else {
    // An image is owned by one queue family at a time and ownership must be
    // explicitly transfered between uses
  ...
  // Create swap chain
  if (vkCreateSwapchainKHR(device->GetVulkanDevice(), &createInfo, nullptr,
                           &vkSwapChain) != VK_SUCCESS) {
    throw std::runtime_error("Failed to create swap chain");
  }

  // --- Retrieve swap chain images ---
  vkGetSwapchainImagesKHR(device->GetVulkanDevice(), vkSwapChain, &imageCount,
                          nullptr);
  ...
  for (size_t i = 0; i < vkSwapChainImages.size(); i++) {
    ...
    if (vkCreateImageView(device->GetVulkanDevice(), &createInfo, nullptr,
  ...
  if (vkCreateSemaphore(device->GetVulkanDevice(), &semaphoreInfo, nullptr,
                        &imageAvailableSemaphore) != VK_SUCCESS ||
      vkCreateSemaphore(device->GetVulkanDevice(), &semaphoreInfo, nullptr,
...
VkExtent2D VulkanSwapChain::GetExtent() const { return vkSwapChainExtent; }

uint32_t VulkanSwapChain::GetIndex() const { return imageIndex; }
...
void VulkanSwapChain::Acquire() {
  ...
  VkResult result = vkAcquireNextImageKHR(
      device->GetVulkanDevice(), vkSwapChain,
...
VulkanSwapChain::~VulkanSwapChain() {
  vkDestroySemaphore(device->GetVulkanDevice(), imageAvailableSemaphore,
                     nullptr);
  vkDestroySemaphore(device->GetVulkanDevice(), renderFinishedSemaphore,
                     nullptr);

  for (size_t i = 0; i < vkSwapChainImageViews.size(); i++) {
    vkDestroyImageView(device->GetVulkanDevice(), vkSwapChainImageViews[i],
                       nullptr);
  }

...
VkRenderPass CreateRenderPass() {
  ...
  if (vkCreateRenderPass(device->GetVulkanDevice(), &renderPassInfo, nullptr,
...
VkDescriptorSetLayout CreateDescriptorSetLayout(
  ...
  vkCreateDescriptorSetLayout(device->GetVulkanDevice(),
...
VkDescriptorPool CreateDescriptorPool() {
  ...
  vkCreateDescriptorPool(device->GetVulkanDevice(), &descriptorPoolInfo,
...
VkDescriptorSet CreateDescriptorSet(VkDescriptorPool descriptorPool,
  ...
  vkAllocateDescriptorSets(device->GetVulkanDevice(), &allocInfo,
                           &descriptorSet);
...
VkPipelineLayout CreatePipelineLayout(
  ...
  if (vkCreatePipelineLayout(device->GetVulkanDevice(), &pipelineLayoutInfo,
...
VkPipeline CreateGraphicsPipeline(VkPipelineLayout pipelineLayout,
  ...
  VkShaderModule vertShaderModule = createShaderModule(
      "5_helloTessellation/shaders/shader.vert.spv", device->GetVulkanDevice());
  VkShaderModule tescShaderModule = createShaderModule(
      "5_helloTessellation/shaders/shader.tesc.spv", device->GetVulkanDevice());
  VkShaderModule teseShaderModule = createShaderModule(
      "5_helloTessellation/shaders/shader.tese.spv", device->GetVulkanDevice());
  VkShaderModule fragShaderModule = createShaderModule(
      "5_helloTessellation/shaders/shader.frag.spv", device->GetVulkanDevice());
  ...
  VkPipeline pipeline;
  if (vkCreateGraphicsPipelines(device->GetVulkanDevice(), VK_NULL_HANDLE, 1,
...
VkPipeline CreateComputePipeline(VkPipelineLayout pipelineLayout) {
  VkShaderModule compShaderModule = createShaderModule(
      "5_helloTessellation/shaders/shader.comp.spv", device->GetVulkanDevice());
  ...
  VkPipeline pipeline;
  if (vkCreateComputePipelines(device->GetVulkanDevice(), VK_NULL_HANDLE, 1,
                               &pipelineInfo, nullptr,
                               &pipeline) != VK_SUCCESS) {
    throw std::runtime_error("Failed to create pipeline");
  }
...
std::vector<VkFramebuffer> CreateFrameBuffers(VkRenderPass renderPass) {
  ...
  for (uint32_t i = 0; i < swapchain->GetCount(); i++) {
    ...
    if (vkCreateFramebuffer(device->GetVulkanDevice(), &framebufferInfo,
...
int main(int argc, char** argv) {
  ...
  instance =
      new VulkanInstance(applicationName, glfwExtensionCount, glfwExtensions);
  ...
  poolInfo.queueFamilyIndex =
      instance->GetQueueFamilyIndices()[QueueFlags::Graphics];
  poolInfo.flags = 0;

  VkCommandPool commandPool;
  if (vkCreateCommandPool(device->GetVulkanDevice(), &poolInfo, nullptr,
  ...
  cameraTransforms.projectionMatrix = glm::perspective(
      static_cast<float>(45 * M_PI / 180), 640.f / 480.f, 0.1f, 1000.f);

  ModelUBO modelTransforms;
  modelTransforms.modelMatrix =
      glm::mat4(1.0);  // glm::rotate(glm::mat4(1.f), static_cast<float>(15 *
                       // M_PI / 180), glm::vec3(0.f, 0.f, 1.f));
  ...
  VkBuffer vertexBuffer = CreateBuffer(
      device,
      VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
      vertexBufferSize);
  unsigned int vertexBufferOffsets[1];
  VkDeviceMemory vertexBufferMemory =
      AllocateMemoryForBuffers(device, {vertexBuffer},
  ...
  {
    char* data;
    vkMapMemory(device->GetVulkanDevice(), vertexBufferMemory, 0,
                vertexBufferSize, 0, reinterpret_cast<void**>(&data));
  ...
  BindMemoryForBuffers(device, vertexBufferMemory, {vertexBuffer},
                       vertexBufferOffsets);

  // Create uniform buffers
  VkBuffer cameraBuffer = CreateBuffer(
      device, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, sizeof(CameraUBO));
  VkBuffer modelBuffer = CreateBuffer(
      device, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, sizeof(ModelUBO));
  unsigned int uniformBufferOffsets[2];
  VkDeviceMemory uniformBufferMemory =
      AllocateMemoryForBuffers(device, {cameraBuffer, modelBuffer},
                               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                   VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
  ...
  {
    char* data;
    vkMapMemory(device->GetVulkanDevice(), uniformBufferMemory, 0,
                uniformBufferOffsets[1] + sizeof(ModelUBO), 0,
                reinterpret_cast<void**>(&data));
    memcpy(data + uniformBufferOffsets[0], &cameraTransforms,
           sizeof(CameraUBO));
  ...
  // Bind the memory to the buffers
  BindMemoryForBuffers(device, uniformBufferMemory, {cameraBuffer, modelBuffer},
                       uniformBufferOffsets);
  ...
  VkDescriptorSet computeSet =
      CreateDescriptorSet(descriptorPool, computeSetLayout);
  VkDescriptorSet cameraSet =
      CreateDescriptorSet(descriptorPool, cameraSetLayout);
  VkDescriptorSet modelSet =
      CreateDescriptorSet(descriptorPool, modelSetLayout);
  ...
  // Initialize descriptor sets
  {
    ...
    VkWriteDescriptorSet writeDescriptorSets[] = {
        writeComputeInfo, writeCameraInfo, writeModelInfo};

    vkUpdateDescriptorSets(device->GetVulkanDevice(), 3, writeDescriptorSets, 0,
                           nullptr);
  }

  VkRenderPass renderPass = CreateRenderPass();
  VkPipelineLayout computePipelineLayout =
      CreatePipelineLayout({computeSetLayout});
  VkPipelineLayout graphicsPipelineLayout =
      CreatePipelineLayout({cameraSetLayout, modelSetLayout});
  VkPipeline computePipeline = CreateComputePipeline(computePipelineLayout);
  VkPipeline graphicsPipeline =
      CreateGraphicsPipeline(graphicsPipelineLayout, renderPass, 0);
  ...
  if (vkAllocateCommandBuffers(device->GetVulkanDevice(),
  ...
  for (unsigned int i = 0; i < commandBuffers.size(); ++i) {
    ...
    vkCmdBindPipeline(commandBuffers[i], VK_PIPELINE_BIND_POINT_COMPUTE,
                      computePipeline);

    // Bind descriptor sets for compute
    vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_COMPUTE,
                            computePipelineLayout, 0, 1, &computeSet, 0,
                            nullptr);

    // Dispatch the compute kernel, with one thread for each vertex
    vkCmdDispatch(commandBuffers[i], vertices.size(), 1, 1);

    // Define a memory barrier to transition the vertex buffer from a compute
    // storage object to a vertex input
    ...
    computeToVertexBarrier.srcQueueFamilyIndex =
        device->GetQueueIndex(QueueFlags::Compute);
    computeToVertexBarrier.dstQueueFamilyIndex =
    ...
    vkCmdPipelineBarrier(commandBuffers[i],
                         ...
                         &computeToVertexBarrier, 0, nullptr);
    ...
    vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                            graphicsPipelineLayout, 0, 1, &cameraSet, 0,
                            nullptr);
    ...
    vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                            graphicsPipelineLayout, 1, 1, &modelSet, 0,
                            nullptr);
  ...
  int frameNumber = 0;
  // Map the part of the buffer referring the camera view matrix so it can be
  // updated when the camera moves
  vkMapMemory(device->GetVulkanDevice(), uniformBufferMemory,
              uniformBufferOffsets[0] + offsetof(CameraUBO, viewMatrix),
              sizeof(glm::mat4), 0,
              reinterpret_cast<void**>(&mappedCameraView));
  while (!ShouldQuit()) {
    swapchain->Acquire();
  ...
  vkDestroyDescriptorSetLayout(device->GetVulkanDevice(), computeSetLayout,
                               nullptr);
  vkDestroyDescriptorSetLayout(device->GetVulkanDevice(), cameraSetLayout,
                               nullptr);
  vkDestroyDescriptorSetLayout(device->GetVulkanDevice(), modelSetLayout,
                               nullptr);
  ...
  vkDestroyPipelineLayout(device->GetVulkanDevice(), computePipelineLayout,
                          nullptr);

  vkDestroyPipeline(device->GetVulkanDevice(), graphicsPipeline, nullptr);
  vkDestroyPipelineLayout(device->GetVulkanDevice(), graphicsPipelineLayout,
                          nullptr);
  vkDestroyRenderPass(device->GetVulkanDevice(), renderPass, nullptr);
  vkFreeCommandBuffers(device->GetVulkanDevice(), commandPool,
